#!/usr/bin/bash
# Adopted from https://github.com/team-plover/warlocks-gambit
...
WasmFile="$(cargo metadata --format-version 1 | sed -n 's/.*"target_directory":"\([^"]*\)".*/\1/p')/wasm32-unknown-unknown/release/examples/$ProjName.wasm"
...
BINDGEN_EXEC_PATH="$HOME/.cargo/bin/wasm-bindgen"
...
cp scripts/test_wasm.html "$OutDir/index.html"
